...
public class TopClass implements ActionListener, KeyListener {
	...
	/**
	 * Implementation for action events-The method takes in an ActionEvent object as a parameter,
	 * If the source of the event is the "startGame" object, the code stops the splash screen by setting the loopVar
	 * variable to false and calls the "fadeOperation()" method.
	 */
	public void actionPerformed(ActionEvent e) {
		...
		/**If the source of the event is the "buildComplete" object, the code creates a new thread and starts it.
		 * Inside the thread, the loopVar variable is set to true, the gamePlay variable is set to true,
		 * and the "gameScreen()" method is called with the parameter false.
		 *
		 */
	...
	private void fadeOperation() {
		Thread t = new Thread() {
			public void run() {
				topPanel.remove(startGame); //removes the start button from the top panel
				topPanel.remove(pgs); //removes the game screen from the top panel
				topPanel.revalidate(); //revalidates the top panel to make sure that the components are properly laid out.
				topPanel.repaint(); //repaints the top panel to make sure that the changes made in the previous lines are visible
				
				//panel to fade
				JPanel temp = new JPanel(); //temp panel is the panel we fade
				int alpha = 0; /*alpha channel variable
				creates an integer variable alpha which will be used to store the current alpha value of the temp panel */
				temp.setBackground(new Color(0, 0, 0, alpha)); //transparent, black JPanel
				topPanel.add(temp); //adds the temp panel to the top panel.
				topPanel.add(pgs); //adds the game screen to the top panel.
				topPanel.revalidate();
				topPanel.repaint();
				
				long currentTime = System.currentTimeMillis();
				//gets the current time in milliseconds and assigns it to the currentTime variable.
				...
				//the build complete object informs the game that its allowed to begin
	...
	private void gameScreen(boolean isSplash) {
		...
		/*variable to hold the loop start time-this variable is updated at the end of each iteration in the game clock
		...
		while(loopVar) {
			if((System.currentTimeMillis() - startTime) > UPDATE_DIFFERENCE) {
				...
				if(birdThrust && !isSplash) {
					playSound("/Users/Rabiii/Downloads/flappyBird/resources/sfx_swooshing.wav");
	 ...
	 * CHECKS WHETHER BIRD HAS PASSED THROUGH PIPE
	 ...
	 * @param bird Bird object
	 * Method responsible for the overall collision detection for the game. It takes in all the pipes and bird objects
	 * as arguments,calls the collisionHelper method for each pipe to check for collisions, even with ground.
	 */
	private void collisionDetection(BottomPipe bp1, BottomPipe bp2, TopPipe tp1, TopPipe tp2, Bird bird) {
		...
		if(bird.getY() + BIRD_HEIGHT > SCREEN_HEIGHT*7/8) { //ground detection
			playSound("/Users/Rabiii/Downloads/flappyBird/resources/die.wav");
	 ...
	 * The collisionHelper(Rectangle r1, Rectangle r2, BufferedImage b1, BufferedImage b2) method is a helper method
	 * that is used by the collisionDetection(BottomPipe bp1, BottomPipe bp2, TopPipe tp1, TopPipe t2, Bird bird) method
	 to check for collisions between the bird and a single pipe. It takes in the rectangles and buffered images of the
	bird and the pipe, and checks if they intersect.
	 */
